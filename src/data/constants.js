import votingImage from "./CMS-App.png";
import proshop from "./Garments-Store.png";
import portfolio from "./portfolio-project.png";

export const Bio = {
  name: "Nadeem Ahmad",
  roles: ["Programmer", "Mern Stack Developer", "Designer"],
  description:
    "I’m a dedicated and an enthusiastic beginner web developer with a intermediate foundation in the MERN Stack and a growing skill set in full-stack development. Passionate about learning, problem-solving and real-world problems through code and love the process of turning ideas into fully functional web solutions. With a mindset focused on continuous learning, I'm always exploring new technologies and improving my skills through hands-on experience. I’m now looking to join a creative and collaborative development team where I can contribute meaningfully, grow professionally, and work on impactful projects that make a difference.",
  github: "https://github.com/nadeembscs2021",
  resume:
    "https://drive.google.com/file/d/1OJpcHSM1mrIZB9MiumrAOTANVe1jVZpg/view?usp=drive_link",
  linkedin:
    "https://www.linkedin.com/in/nadeem-khan-529b4326a?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app",
  twitter: "https://x.com/NadeemKhan_2021?t=ZexRV3G3pyLRBDQ13W9UUg&s=09",
  insta: "https://www.instagram.com/nadeembscs2021?igsh=MjQ5aGYxZjFjbmRp",
  facebook:
    "https://www.facebook.com/profile.php?id=100071757684762&mibextid=ZbWKwL",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "Tailwind CSS",
        image: "https://www.svgrepo.com/show/333609/tailwind-css.svg",
      },
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "Redux",
        image:
          "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      },
      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "SQL",
        image: "https://icon.icepanel.io/Technology/svg/Azure-SQL-Database.svg",
      },
    ],
  },
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://img.icons8.com/?size=100&id=20906&format=png&color=000000",
      },
      {
        name: "GitHub",
        image:
          "https://img.icons8.com/?size=100&id=CexFs1lac6J7&format=png&color=000000",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Postman",
        image: "https://www.svgrepo.com/show/354202/postman-icon.svg",
      },
      {
        name: "Vercel",
        image: "https://www.svgrepo.com/show/327408/logo-vercel.svg",
      },
      {
        name: "Netlify",
        image: "https://www.netlify.com/v3/img/components/logomark.png",
      },
    ],
  },
];

export const experiences = [
  {
    id: 1,
    img: "", // Replace with frontend image link
    role: "Front-End Developer",
    company: "Anees Software House",
    date: "August 2022 – June 2023",
    desc: "Designed and developed responsive websites and portfolios using HTML, CSS, JavaScript, and React.js, and modern CSS frameworks like Tailwind CSS. Specialized in creating cross-device compatible interfaces with optimized performance and intuitive UX. Implemented form validation systems and component-based architectures while maintaining code quality and security standards.",
    skills: ["HTML", "CSS", "JavaScript", "React Js", "Tailwind CSS"],
    doc: "",
  },
  {
    id: 2,
    img: "", // Replace with backend image link
    role: "Back-End Developer",
    company: "Self-Learner (Independent Study)",
    date: "2024 – Present",
    desc: "Developed and deployed backend systems using Node.js, Express.js, MongoDB and SQL, building RESTful APIs that handle authentication, data processing, and database operations. Implemented JWT authentication and authorization in personal projects, while continuously learning and expanding my knowledge of backend architecture and best practices",
    skills: ["Node Js", "Express Js", "MongoDB", "SQL"],
    doc: "",
  },
];

export const education = [
  {
    id: 0,
    img: "https://cms.uom.edu.pk/web/image/website/1/logo/University%20of%20Malakand?unique=ce6444b",
    school: "University of Malakand (UOM)",
    date: "Sep 2021 - Sep 2025",
    grade: "3.5 CGPA",
    desc: "I have done my bachelor's degree in Computer Science, gaining a strong foundation in programming, algorithms, data structure and software development. Developed problem-solving skills through coursework and practical projects in various computer science disciplines.",
    degree: "Bachelor of Science in Computer Science",
  },
  {
    id: 1,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRCzOz8QA1ZA_C-wtoGRmDUCRD6Q6dyqRWICEQEOYXWlAsMa2Q1H7D7mcCv8SoqRojPpAo&usqp=CAU",
    school: "Government Postgraduate College Timergara",
    date: "Sep 2019 - July 2021",
    grade: "73%",
    desc: "I have successfully completed my FSc in Pre-Engineering at Government Postgraduate College Timergara, showcasing my dedication to academic excellence and a strong grounding in engineering principles.",
    degree: "Pre-engineering(XII)",
  },
  {
    id: 2,
    img: "https://cdn.brandfetch.io/idajlo2Nzf/w/400/h/400/theme/dark/icon.jpeg?c=1bfwsmEH20zzEfSNTed",
    school: "Government High School (TKS)",
    date: "Apr 2016 - Apr 2018",
    grade: "80%",
    desc: "I have successfully finished my Matriculation from our village Government High School Takora Sheikhan, I established a solid academic base that has been instrumental in supporting my subsequent educational endeavors.",
    degree: "Matric in Science",
  },
];

export const projects = [
  {
    id: 9,
    title: "CMS Web App",
    date: "Oct 2024 - July 2025",
    description:
      "This project, built with the MERN stack, offers a secure and efficient platform for managing academic and administrative tasks. Students and parents access real-time updates on performance and fees, while teachers handle schedules and grading seamlessly. Administrators benefit from dynamic dashboards, reporting, and centralized control, providing a scalable solution for educational institutions.",
    image: `${votingImage}`,
    tags: ["Next Js", "React Js", "Redux", "Node Js", "Express Js", "MongoDB"],
    category: "web app",
    // github: "paste github web app project repository link here",
    github: "https://github.com/nadeembscs2021",
    // webapp: "insert web app link here",
  },
  {
    id: 0,
    title: "Online Garments Store (Web App)",
    date: "May - 2025",
    description:
      "Online Garments Store is a full-stack web application built using HTML, CSS, JavaScript, Node.js, Express.js, andSQL. It features a user-friendly interface for customers and a separate admin panel for managing products, users, and orders. The system supports user authentication, dynamic routing, shopping cart, and order tracking with full backend and database integration.",
    image: `${proshop}`,
    tags: ["HTML", "CSS", "JavaScript", "Node Js", "Express Js", "SQL"],
    category: "web app",
    // "paste github portfolio project repository link below",
    github: "https://github.com/nadeembscs2021/nadeem-portfolio",
    webapp: "https://nadeemahmad-portfolio.vercel.app/",
  },
  {
    id: 0,
    title: "React.js Portfolio",
    date: "Sep 2024 - Oct 2024",
    description:
      "A dynamic and responsive portfolio built with React.js, showcasing projects, skills, and achievements. Features smooth navigation, modern UI/UX design, and reusable components, offering an interactive user experience.",
    image: `${portfolio}`,
    tags: ["React Js", "Tailwind CSS", "Material UI"],
    category: "web app",
    // "paste github Mini Portfolio project repository link below",
    github: "https://github.com/nadeembscs2021/Nadeem_Portfolio",
    webapp: "https://nadeembscs2021.github.io/Nadeem_Portfolio/",
  },
];

export const TimeLineData = [
  { year: 2019, text: "Started my journey" },
  { year: 2022, text: "Worked as a freelance developer" },
  { year: 2023, text: "Founded JavaScript Mastery" },
  { year: 2024, text: "Shared my projects with the world" },
  { year: 2025, text: "Started my own platform" },
];
